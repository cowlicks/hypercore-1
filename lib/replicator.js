...
class Peer {
  ...
  _maybeWant (start, length = 1) {
    ...
    console.log('DEFAULT_SEGMENT_SIZE', DEFAULT_SEGMENT_SIZE);
    if (start + length <= this.remoteContiguousLength) return

    let i = Math.floor(start / DEFAULT_SEGMENT_SIZE)
    console.log('let i = Math.floor(start / DEFAULT_SEGMENT_SIZE)', i);
    const n = Math.ceil((start + length) / DEFAULT_SEGMENT_SIZE)
    console.log(
    'const n = Math.ceil((start + length) / DEFAULT_SEGMENT_SIZE)', n);
